package com.reversefold.glitch.server.player {
	...
	public class Player extends Common {
		...
		public var newbie_island_butterfly_massaged;
		public var starting_instance;
		public var last_location;
		public var tp_queue;
		public var last_overlay_script;
		public var is_afk;
		public var death_recognized;
		public var time_played;
		public var did_start_move;
		public var dy;
		public var isLoggingIn;
		public var last_online_gameday;
		public var food_today;
		public var radiation_today;
		public var butlers_zombied_today;
		public var skip_entire_intro;
		public var skip_newux;
		public var map_path;
		public var previous_map_path;
		public var grave_marker;
		public var kindly_randomness;
		public var giants_donated;
		public var fully_grown_trants;
		public var musicblocks_heard;
		public var soaked;
		public var butterflies_milked;
		public var too_much_nostalgia_prompt;
		public var run_overlay_onmove;
		public var prefs;
		public var followee;
		public var greeting_graduated;
		public var successful_greets;
		public var unsuccessful_greets;
		public var is_in_timeout;
		public var is_in_help_coneofsilence;
		public var is_in_coneofsilence;
		public var naughty_splanking;
		public var p2p_collisions;
		public var guide_on_duty;
		public var is_deleted;
		public var hasBeenWarned;
		public var greeting_muted;
		
		//RVRS: TODO: unused?
		public var seenMaxDonationsPrompt;
		public var house;
		...
		public function apiPlayerHasLockForCurrentLocation() {
			//RVRS: TODO
			throw new Error('apiPlayerHasLockForCurrentLocation');
		}
		
		public function apiMoveOfflinePlayerToAnotherLocation(target, x, y) {
			//RVRS: TODO
			throw new Error('apiMoveOfflinePlayerToAnotherLocation ' + target + ' ' + x + ' ' + y);
		}
		
		public function apiSetXY(x, y) {
			//RVRS: TODO
			throw new Error('apiSetXY ' + x + ' ' + y);
		}
		
		public function apiCheckIfNeedToMoveToAnotherGSAndGetMoveData(tsid, x, y) {
			//RVRS: TODO
			throw new Error('apiCheckIfNeedToMoveToAnotherGSAndGetMoveData ' + tsid + ' ' + x + ' ' + y);
		}
		
		public function apiStartLocationMoveX(target, x, y) {
			//RVRS: TODO
			throw new Error('apiStartLocationMoveX ' + target + ' ' + x + ' ' + y);
		}
		
		public function apiDelete() {
			throw new Error('apiDelete');
		}
		
		public function apiRemoveAllFollowers() {
			//RVRS: TODO
			throw new Error('apiRemoveAllFollowers');
		}
		
		public function apiStopFollowing() {
			//RVRS: TODO
...
public function acl_keys_build_client_msg(msg=null){
	this.acl_keys_init();
...
public function newxp_move_along(details){
	this.prompts.prompts_add_simple('Time to move along. Try going right?', 30);
}

public function newxp_testing_modal_callback(value, details){
	if (value == 'ok'){
		this.houses.houses_remove_all();
		this.imagination.imagination_reset();
		this.stats.stats_reset_imagination();
		this.stats.stats_reset_xp();
		this.stats.stats_reset_street_history();
		this.skills.skills_reset();
		this.metabolics.metabolics_set_tank(100);
		this.metabolics.metabolics_set_max('energy', 100);
		this.metabolics.metabolics_set_energy(95);
		this.metabolics.metabolics_set_mood(95);
		this.stats.stats_set_quoin_multiplier(1.0);
		this.stats.stats_set_currants(0);
		this.achievements.achievements_reset();
		this.achievements.achievements_reset_daily();
		this.quests.quests_reset();
		this.counters.counters_reset();
		this.physics.setDefaultPhysics();
		this.bag.emptyBag();
		//this.furniture.furniture_reset(); // This deletes and re-creates your furn bag, which confuses the client. do the next thing instead
		var furn_bag = this.furniture.furniture_get_bag();
		if (furn_bag) furn_bag.emptyBag();
		this.physics.physicsReset();
		this.clearPath();

		this.announcements.announce_music_stop(this.location.geometry.music_file, 1);
		this.goToNewNewStartingLevel();
	}
}

public function newxpQuestReminder(){
	if (this.quests.getQuestStatus('buy_two_bags') != 'done' && this.location.class_tsid == 'newbie_island'){
...
public function newxpComplete(){
	...
	this.intro_steps = null; // Is this ok?
	this.newbie_island_butterfly_massaged = null;
	this.newxp_allow_home = null; // no longer needed

	var exterior = this.houses.houses_get_external_street();
	if (exterior){
		exterior.homes_unset_newxp();
	}

	var interior = this.houses.houses_get_interior_street();
	if (interior){
	...
	/*
	var level = this.stats.stats_get_level();
	...
	this.stats.stats_get_login(rsp.stats);
	this.metabolics.metabolics_get_login(rsp.stats);
...
public function newxpReturnToGentleIsland(){
	...
	this.announcements.announce_vog_fade("Though you are on your own now, you are not really alone.//You should find that the other players are pretty helpful, so if you have any questions, just ask.", {width: 600, done_payload: {location_script_name: 'openContacts'}});
}
...
public function newxpGiveGreeterTwig(){
	this.announcements.overlay_dismiss('greeter_twig');

	this.announcements.announce_vog_fade("It’s a Greeter Twig! You can use it to summon another player to come and say hi.", {css_class: 'nuxp_big', fade_alpha: 0.7, done_payload: {function_name: 'newxpExplainGreeters'}});
	this.items.createItemFromFamiliar('greeter_twig', 1);
}

public function newxpExplainGreeters(){
	this.announcements.announce_vog_fade("(Greeters are real people — other players who volunteer to welcome new ‘uns — and tend to be nice. Enjoy, and use the twig whenever you want.)", {css_class: 'nuxp_medium', fade_alpha: 0.7});
}
...
public function checkNeighborSignpost(){
	if (this.quests.getQuestStatus('leave_gentle_island') == 'done' && this.buddies.buddies_count()){
		var exterior = this.houses.houses_get_external_street();
		if (exterior){
...
public function init(){
	...
	this.bag_size = null;
	this.last_pet = null;
	//this.houses.home = null;
	*/

	if (this.starting_instance) this.starting_instance.apiDelete();
	this.starting_instance = null;

	if (!this.is_god && this.teleportation.location_history) this.teleportation.location_history = null;
}
...
public function onLogin(){
	...
	this.familiar.familiar_clean_queue();
	this.quests.quests_restart_queue();

	// Clear Zilloween data if necessary
	if (!isZilloween()) {
		this.items.clearZilloweenData();
	}
	...
	var tsids = this.buddies.buddies_get_reverse_tsids();
	var args = {
		tsid: this.tsid,
		label: this.label,
		level: this.stats.stats_get_level(),
		location: {
	...
	this.imagination.imagination_convert();
	//this.imagination.imagination_convert_currants();
}
...
public function onLoggedin(){
	...
	this.buffs.buffs_login();
	this.houses.houses_login();

	//
	// learn new recipes created since we learned the skill
	//

	this.skills.skills_learn_missing_recipes();
	this.achievements.achievements_login();
	this.quests.quests_login();
	this.party.party_login();
	this.instances.instances_login();
	this.trading.trading_rollback();
	this.rewards.rewards_return();
	this.conversations.conversations_login();
	...
	if (this.last_overlay_script){
		this.last_overlay_script = null;
		this.announcements.run_overlay_script(this.last_overlay_script);
	...
	if (this.death_recognized) {
		...
		this.death_recognized = null;
	}

	// fix broken quoin upgrades:
	var coin_modifier = 0;
	if (this.buffs.buffs_has('crazy_coin_collector')) coin_modifier = 50;

	this.stats.stats_set_quoins_max_today(this.imagination.imagination_get_quoin_limit() + coin_modifier);
...
public function onLogout(){
	...
	this.buffs.buffs_logout();
	this.houses.houses_logout();
	this.party.party_logout();
	this.instances.instances_logout();
	this.groups.groups_logout();
	this.organizations.organizations_logout();
	this.trading.trading_cancel_auto();
	this.quests.quests_pause_queue();
	this.quests.quests_on_logout();
	...
	if (this.date_last_loggedin && (this.date_last_loggedin > this.date_last_logout || !this.date_last_logout)){
		...
		this.achievements.achievements_increment('time_played', day, time_played);
		this.counters.counters_increment('time_played', day, time_played);
	...
	if (this['!mining']){
		var rock = Server.instance.apiFindObject(this['!mining']);
		if (rock){
	...
	if (this['!clearing']){
		var tree = Server.instance.apiFindObject(this['!clearing']);
		if (tree){
	...
	if (this['!pulling']) {
		var cracker = Server.instance.apiFindObject(this['!pulling']);
		if (cracker){
	...
	if (this.location.isInstance()){
		if (this.location.instance_id.substr(0, 19) == 'underground_puzzle_' || this.location.instance_id.substr(0, 19) == 'ticket_to_paradise_'){
			var exit = this.instances.instances_get_exit(this.location.instance_id);
			this.teleportToLocationDelayed(exit.tsid, exit.x, exit.y);
			this.prompts.prompts_add_simple("It looks like you ran into some trouble. You've been moved to a safe place.");
		}
	}
}
...
public function didStartStreetMove(){
	this.did_start_move = true;

	/* Update data for hairball achievement */
	if (this.buffs.buffs_has('hairball_dash')) {
		this.buffs.buffs_hairball_leave_street();
	}
	return true;
}

public function didEndStreetMove(){
	this.did_start_move = null;
	return true;
}

public function isMovingStreets(){
...
public function sendOnlineActivity(msg){
	this.sendActivity(msg);
}

public function sendLocationActivity(msg, from, exclusions){
...
public function teleportToLocation(tsid, x, y, args=null){
	...
	if (tsid != this.location.tsid && target.instances_instance_me() && !args.ignore_instance_me){
		...
		if (!instance){
			...
			instance = this.instances.instances_create(instance_id, target.tsid, instance_options, location_options);

			target.instances_add_instance(instance_id, instance);
		}
		else if (!this.instances.instances_has(instance_id)){
			this.instances.instances_add(instance_id, instance);
		}

		//pc.instances_enter(instance_id, connect.x, connect.y);
		this.instances.previously[instance_id] = this.get_simple_location();
		target = Server.instance.apiFindObject(instance.get_entrance());
	}
	...
	if (target.getProp('hubid') != this.location.getProp('hubid')){
		this.announcements.announce_sound('TELEPORT_AWAY_HUB');
	}else{
		this.announcements.announce_sound('TELEPORT_AWAY');
	}
...
public function teleport_to_player(target){
...
public function onTimePlaying(){
	...
	if(!this['!in_house_deco_mode'] && !this.location.isParadiseLocation() && ((!this['!meditating'] || this['!meditating'] != 'transcendental'))) {
		if (!this.location.is_newxp){
			...
			if (this.buffs.buffs_has('rookswort')){
				mood_loss_percentage_multiplier = 0.5;
			}

			var max_mood = Math.min(this.metabolics.metabolics_get_max_mood(), 2000);

			var mood_loss = Math.max(max_mood * (0.0025 * mood_loss_percentage_multiplier), 1);
			var mood_percentage = this.metabolics.metabolics_get_percentage('mood');

			if (mood_percentage > 80){
				mood_loss = Math.max(max_mood * (0.015 * mood_loss_percentage_multiplier), 4);

				if (this.imagination.imagination_has_upgrade('periodic_mood_loss_reduced_3')){
					mood_loss = Math.max(max_mood * (0.0075 * mood_loss_percentage_multiplier), 4);
				}else if (this.imagination.imagination_has_upgrade('periodic_mood_loss_reduced_2')){
					mood_loss = Math.max(max_mood * (0.010 * mood_loss_percentage_multiplier), 4);
				}else if (this.imagination.imagination_has_upgrade('periodic_mood_loss_reduced_1')){
			...
			if (!this.has_done_intro){
				if (this.metabolics.metabolics_get_percentage('mood') <= 50) mood_loss = 0;
			}

			if (mood_loss) this.metabolics.metabolics_lose_mood(mood_loss, 1);
		}
	}
	...
	if (this.metabolics.metabolics_get_percentage('energy') <= 10 && !this.buffs.buffs_has('walking_dead') && !this.is_dead){
...
public function checkNewGameDay(){
	...
	if (today > this.last_online_gameday){
		...
		else if (this.quests.end_esquibeth ||
...
public function getTimePlayed(){
	return this.counters.counters_get_group_sum('time_played');
}

public function doNewDay(){
	//
	...
	var mood = 0;
	if (this.has_done_intro){
		energy = this.metabolics.metabolics_set_energy(this.metabolics.energy.top);
		mood = this.metabolics.metabolics_set_mood(this.metabolics.mood.top);
	}

	Server.instance.apiLogAction('NEW_DAY', 'pc='+this.tsid, 'energy='+energy, 'mood='+mood);

	var maxDonationXP = Math.round(this.stats.stats_calc_level_from_xp(this.stats.stats_get_xp()).xp_for_this * 0.1);
	...
	if (this.isOnline() && (previous_time_played + session_time_played) >= 3600 && this.has_done_intro){
		...
		var rsp = {
			...
			"stats_yesterday"	: {
				energy: intval(this.daily_history.daily_history_get(yesterday_key, 'energy_consumed')) * -1,
				imagination: intval(this.daily_history.daily_history_get(yesterday_key, 'imagination'))
			},
			"stats_today"		: {
				energy: energy,
				mood: mood
		...
		var hi_variants_tracker = Server.instance.apiFindObject(config.hi_variants_tracker);
		...
		var levels = intval(this.daily_history.daily_history_get(yesterday_key, 'level_up'));
		if (levels) rsp.new_level = this.stats.stats_get_level();

		var quests = this.daily_history.daily_history_get(yesterday_key, 'quests_completed');
		if (num_keys(quests)){
			rsp.complete_quests = [];
			for (var i in quests){
				var q = this.quests.getQuestInstance(quests[i]);
				if (q){
		...
		var collections = this.daily_history.daily_history_get(yesterday_key, 'collections');
		if (num_keys(collections)){
			rsp.complete_collections = [];
			for (var i in collections){
				var c = this.achievements.achievements_get(collections[i]);
				if (c){
					rsp.complete_collections.push({name: c.name, tsid: this.achievements.achievement_get_trophy(collections[i]).class_tsid});
				}
			}
		}

		var skills = this.daily_history.daily_history_get(yesterday_key, 'skills_learned');
		if (num_keys(skills)){
			rsp.new_skills = [];
			for (var i in skills){
				rsp.new_skills.push({name: this.skills.skills_get_name(skills[i]), tsid: skills[i]});
			}
		}

		var achievements = this.daily_history.daily_history_get(yesterday_key, 'achievements');
		if (num_keys(achievements)){
			rsp.new_badges = [];
			for (var i in achievements){
				var a = this.achievements.achievements_get(achievements[i]);
				if (a){
					rsp.new_badges.push({
						name: a.name,
						tsid: achievements[i],
		...
		this.daily_history.daily_history_archive(yesterday_key);
		this.daily_history.daily_history_reset();
	}

	// Quoins
	if (this.stats.quoins_today.value >= 100){
		this.achievements.achievements_increment('coin_count', 'days_maxed');
	}
	else{
		this.achievements.achievements_set('coin_count', 'days_maxed', 0);
	}

	this.stats.stats_set_quoins_today(0);

	// fix broken quoin upgrades:
	this.stats.stats_set_quoins_max_today(this.imagination.imagination_get_quoin_limit());

	if (this.buffs.buffs_has('crazy_coin_collector')){
		this.buffs.buffs_remove('crazy_coin_collector');
	}

	// Reset daily rube trades
	this.stats.stats_set_rube_trades(0);
	this.stats.stats_set_rube_lure_disabled(false);

	// Food counter
	this.food_today = 0;

	// Meditation counters
	this.stats.stats_set_meditation_today(0);
	...
	if (this.butlers_zombied_today) {
		this.butlers_zombied_today = null;
		this.quests.quests_set_counter("butlers_zombied", 0);
	}

	// Reset daily favor limits
	for(var i = 0; i < config.base.giants.length; ++i) {
		var giant = config.base.giants[i];
		this.stats.daily_favor[giant] = new Prop(0, 0, this.stats.stats_get_max_favor(giant));
		this.stats.daily_favor[giant].apiSet(0);
	}

	// reset daily donation xp limits
	this.stats.donation_xp_today = new Prop(0, 0, maxDonationXP);
	...
	this.teleportation.teleportation_reset_counters();

	// Mining gem drop overrides
	this.skill_packages.removeSkillPackageOverride('mining');
	this.skill_packages.removeSkillPackageOverride('mining_fancypick');

	if (this.buffs.buffs_has('impervious_miner')){
		this.skill_packages.addSkillPackageOverride('mining', {energy_cost: 0});
		this.skill_packages.addSkillPackageOverride('mining_fancypick', {energy_cost: 0});
	}

	this.stats.recipe_xp_today = null;

	// Reset daily counters
	this.stats.stats_reset_daily_counter();

	this.emotes.hi_emote_variant = null;
}
...
public function deleteMe(){
		...
		for (var i in this.friends){
			if (this.friends[i].pcs){
				for (var j in this.friends[i].pcs){
					...
					this.buddies.removeBuddy(this.friends[i].pcs[j].tsid);
				}
			}
		}
	...
	this.rewards.rewards_reset();
	this.prompts.prompts_delete();
	this.auctions.auctions_delete();
	this.houses.houses_remove_all();
	this.familiar.familiar_delete();
	this.quests.quests_delete();
	this.skills.skills_delete();
	this.stats.stats_delete();
	this.making.making_delete();
	this.buffs.buffs_delete();
	this.achievements.achievements_delete_all();
	this.instances.instances_delete_all();
	this.groups.groups_delete_all();
	this.organizations.organizations_delete_all();
	this.mail.mail_delete();
	this.conversations.conversations_delete();

	this.apiDelete();
}

public function findAllCloseStacks(target_class_tsid, max_distance, args) {
...
public function findCloseStack(target_class_tsid, max_distance, args){
...
public function onCreate(){
	...
	if (this.skip_newux){
		this.stats.stats_set_currants(1000);
		this.metabolics.metabolics_set_mood(80);
		this.metabolics.metabolics_set_energy(80);
		this.stats.stats_add_xp(100);

		this.items.createItem('hoe', 1);
		this.items.createItem('watering_can', 1);
		this.items.createItem('sammich', 3);
		this.items.createItem('papl_upside_down_pizza', 3);
		this.items.createItem('common_crudites', 3);
		this.items.createItem('milk_butterfly', 3);
		this.items.createItem('bubble_tea', 3);
		this.items.createItem('cloud_11_smoothie', 3);

		this.skip_newux = null;
		this.has_done_intro = 1;
		//this.teleportToLocationDelayed('LLIF6R3R9GE1GQB', -412, -112); // burnabee
		return;
	}

	this.stats.stats_set_currants(0); // formerly 100: http://bugs.tinyspeck.com/10801
	this.metabolics.metabolics_set_mood(55);
	this.metabolics.metabolics_set_energy(55);
	...
	this.physics.stacked_physics_cache = null;
...
public function croak(){
	...
	if (this.is_dead || this.buffs.buffs_has('walking_dead') || this.location.isInHell()) return 0;
	if (!this.has_done_intro || this.location.class_tsid == 'newbie_island') return 0;

	Server.instance.apiLogAction('CROAKED', 'pc='+this.tsid);
	this.announcements.apiSendAnnouncement({type: 'stop_all_music'});
	this.announcements.announce_sound('CROAK');

	if (!this.deaths_today) this.deaths_today = 0;
	this.deaths_today++;
	this.sendOnlineActivity('You croaked!');
	this.metabolics.metabolics_lose_mood(this.metabolics.metabolics_get_mood());
	this.metabolics.metabolics_lose_energy(this.metabolics.metabolics_get_energy());
	...
	if (this.location.is_transit){
		...
		if (this.location == this.houses.home.interior) {
			this.achievements.achievements_increment("croaked", "at_home", 1);
		}
	}
	...
	this.achievements.achievements_increment('player', 'deaths');


	if (this.map_path) this.previous_map_path = this.map_path;

	//this.quests.quests_offer('you_croaked', true);

	if (this.buffs.buffs_has('no_no_powder_crash')){
		this.buffs.buffs_remove('no_no_powder_crash');
	}

	if (this.buffs.buffs_has('no_no_powder')){
		this.buffs.buffs_remove('no_no_powder');
	}
...
public function setGraveMarker(marker) {
	this.grave_marker = marker;
}
public function removeGraveMarker() {
	if (this.grave_marker) {
		this.grave_marker = null;
	}
}
...
public function resurrect(){
	...
	//log.info('---------------- resurrect');
	this.is_dead = 0;
	this.grapes_squished = null;
	if (this.grave_marker) {
		this.grave_marker.onDisappear();
	}

	this.announcements.announce_music_stop('HELL', 2);

	if (this.buffs.buffs_has('pooped')){
		this.metabolics.metabolics_set_energy(this.metabolics.energy.top * 0.05);
	}else{
		this.metabolics.metabolics_set_energy(this.metabolics.energy.top * 0.10);
	}
	if (this.metabolics.metabolics_get_mood() > (this.metabolics.mood.top / 2)) this.metabolics.metabolics_set_mood(this.metabolics.mood.top / 2);

	var dest;
	var prev;
	if (this.location.isInstance('hell_one')){
		prev = this.instances.instances_get_exit('hell_one');
		if (prev) dest = Server.instance.apiFindObject(prev.tsid);
	}
	else{
		// wtf?
		dest = this.resurrect_location ? Server.instance.apiFindObject(this.resurrect_location.tsid) : null;
	...
	this.quests.quests_set_flag('resurrected');
...
//	this.buffs.buffs_apply('pooped', {duration: remaining});
...
public function getDeathRecognitionDetails() {
	return this.death_recognized;
}

public function removeDeathRecognitionDetails() {
    if (this.death_recognized) {
        this.death_recognized = null;
	}
}

public function recognizeDeath(type, recognizer) {
	...
	if (this.isOnline()) {
		if (this.location.isInHellOne() && this.quests.getQuestStatus('hell_hole') == 'none' && this.quests.getQuestStatus('hell_quest') == 'none'){
			this.quests.quests_offer('hell_hole', true);
		}else if (this.location.isInPurgatoryHub() && this.quests.getQuestStatus('hell_quest') == 'none'){
			this.quests.quests_offer('hell_quest', true);
		}else{
			var activityText = "";
			...
			var remaining = this.items.createItemFromFamiliar('drink_ticket', 1);
			if (remaining) {
...
public function respondToFollow(choice, details) {
...
public function get_simple_location(){
	return {
		tsid:	this.location.tsid,
		x:	this.x,
		y:	this.y
...
public function goToNewNewStartingLevel(){
	...
	var instance = this.instances.instances_create(instance_id, tsid, {no_auto_return: true, location_type: instance_id});

	// Create the second instance and add it to the player
	var instance2 = this.instances.instances_create('newxp_training1', config.newxp_locations['newxp_training1'], {no_auto_return: true, location_type: 'newxp_training1'});

	// Create the third instance and add it to the player
	var instance3 = this.instances.instances_create('newxp_training2', config.newxp_locations['newxp_training2'], {no_auto_return: true, location_type: 'newxp_training2'});

	// Go to the first instance
	this.instances.instances_enter(instance_id, 609, -6100, true);
}

public function goToFallingLevel(exit){

	var tsid = config.is_dev ? 'LHH12F9DO801RUL' : 'LLI12DVTLA01HT5';
	var instance = this.instances.instances_create('falling', tsid, {instance_label: 'You Feel Woooooozy...', no_auto_return: true});
	if (instance){
		if (exit && exit.tsid){
			var loc = Server.instance.apiFindObject(instance.get_entrance());
		...
		this.instances.instances_enter('falling', 0, -10000);
	}
}

// Find the best greeting/intro location for new players
public function getGreetingLocation(){
	...
	for (var i=0; i<config.greeting_locations.length; i++){
		var loc = Server.instance.apiFindObject(config.greeting_locations[i].tsid);
		if (loc){
...
public function greeterGoToGreetingLocation(){
...
public function resetForTesting(skip=false){
	...
	this.houses.houses_remove_all();
	this.familiar.familiar_reset();
	this.quests.quests_reset();
	this.skills.skills_reset();
	this.stats.stats_delete();
	this.making.making_reset();
	this.buffs.buffs_reset();
	this.achievements.achievements_reset();
	this.trophies.trophies_reset();
	this.instances.instances_reset();
	this.teleportation.teleportation_reset();
	this.daily_history.daily_history_reset();
	//this.jobs.jobs_reset(); // Maybe not do this? Since we're not resetting jobs
	this.auctions.auctions_reset(true);
	this.rewards.rewards_reset();
	this.mail.mail_reset();
	this.conversations.conversations_reset();
	this.trading.trading_reset();
	this.prompts.prompts_reset();
	this.buddies.buddies_reset_cache();
	this.counters.counters_reset();
	this.imagination.imagination_reset();

	// reset other stats and stuff
	this.stats.stats_reset_xp();
	this.stats.stats_reset_favor();
	this.stats.stats_reset_street_history();
	this.stats.currants.apiSet(0);
	this.metabolics.metabolics_recalc_limits();
	this.metabolics.metabolics_set_energy(100);
	this.metabolics.metabolics_set_mood(100);
	...
	this.kindly_randomness = null;
	this.giants_donated = null;
	this.physics = null;
	this.fully_grown_trants = null;
	this.musicblocks_heard = null;
	this.soaked = null;
	this.house = null;
	this.skill_packages.skill_package = null;
	this.events = null;
	this.teleportation.location_history = null;
	this.stats.quoins_today = null;
	this.stats.meditation_today = null;
	this.stats.giant_emblems = null;
	this.special_locations.baqala_times = null;
	this.physics.physics_new = null;
	this.avatar = null;
	this.map_path = null;
	this.butterflies_milked = null;
	this.too_much_nostalgia_prompt = null;
	this.intro_steps = null;
	...
	if (tokens_to_refund){
		this.teleportation.teleportation_give_tokens(tokens_to_refund, "Imbued Teleportation Script refund");
	}

	// Restore items
	for (var i in items_to_restore){
		this.items.createItemFromFamiliar(items_to_restore[i], 1);
	}
...
public function distanceFromPlayerXY(x, y){
...
public function performPostProcessing(msg){
	...
	if (this['!meditating']){
		...
		if (interrupt && !in_array(msg.type, meditation_ok_copy) && !in_array(msg.verb, this.meditation_ok_verbs)){
			...
			var orbs = this.items.get_stacks_by_class('focusing_orb');
			for (var i in orbs){
	...
	if (msg.type == 'move_xy' && this['!moved_lat'] >= 150){
		if (this['!dismiss_overlay']){
			this.events.events_add({overlay: this['!dismiss_overlay'], callback: 'dismiss_overlay_event'}, 1);
			delete this['!dismiss_overlay'];
		}
		else if (this.run_overlay_onmove){
			this.events.events_add({overlay: this.run_overlay_onmove, callback: 'run_overlay_event'}, 1);
			this.run_overlay_onmove = null;
		}
...
public function dismiss_overlay_event(details){
	this.apiSendMsgAsIs({type: 'overlay_cancel', uid: details.overlay});
}

public function run_overlay_event(details){
	this.announcements.run_overlay_script(details.overlay);
}
...
public function playEmotionAnimationDelayed(details){
...
public function playDoAnimation(ms){
	this.apiSendMsgAsIs({
		type:	'play_do',
...
public function stopDoAnimation(){
	this.apiSendMsgAsIs({
		type:	'play_do',
		stop: 'true'
	});
...
public function playHitAnimation(hit_type, ms=null){
...
public function stopHitAnimation(){
	this.apiSendMsgAsIs({
		type:	'play_hit',
		stop: 'true'
	});
...
public function moveCamera(x, from_edge){
	var rsp = {
		type:	'camera_offset',
		x:	intval(x)
	};
...
public function centerCamera(pt, duration){
	this.apiSendMsg({
		type: 'camera_center',
		pt: pt,
...
public function make_hash(){
	return {
		...
		home_info	: this.houses.houses_get_login_new()
	};
}
...
public function make_hash_with_location(){
	var out = {
		...
		rs	: this.rook.isRooked(),
		is_admin	: (this.is_god || this.is_help) ? true : false,
		is_guide	: this.is_guide ? true : false,
		home_info	: this.houses.houses_get_login_new()
	};

	out['physics_adjustments'] = this.physics.reducePhysicsAdjustmentsToMinumum(this.physics.getStackedPhysics());

	var props = this.avatar.avatar_get_pc_msg_props();
	for (var i in props) out[i] = props[i];

	this.games.games_add_msg_props(out);
...
public function getCameraAbilities(){
	var range = 0;
	var can_snap = true;
	var disabled_reason = '';
	...
	if (!this.skills.skills_has('eyeballery_1')){
		range = 400;
	...
	if (!this.imagination.imagination_has_upgrade('snapshotting')){
		can_snap = false;
...
public function sendCameraAbilities(){
	this.apiSendMsg({
...
public function getPrefs() {
	...
	if ('no_oneclick_pickup' in this.prefs) {
		...
		this.prefs.no_oneclick_pickup = null;
	}
...
public function setPrefs(new_prefs) {
	if (new_prefs) {
...
public function openInputBox(uid, title, args){
...
public function inputBoxResponse(msg){
	// An item asked for this. Tell them.
	if (msg.itemstack_tsid){
		var item = Server.instance.apiFindObject(msg.itemstack_tsid);
		...
		if (msg.value){
			...
			var org = this.organizations.organizations_get();
			if (org){
...
public function reloadGeometry(x, y, type){
	...
	if (this.location.geometry.music_file){
		this.announcements.apiSendAnnouncement({type: 'stop_all_music'});
		this.announcements.announce_music(this.location.geometry.music_file, 999);
	}
...
public function onPeriodicEnergyLoss(){
	...
	if (!this['!in_house_deco_mode'] && !this.location.isParadiseLocation() &&  ((!this['!meditating'] || this['!meditating'] != 'transcendental'))){
		...
		if (this.has_done_intro){
			...
			to_lose = Math.round(this.metabolics.metabolics_get_max_energy() * 0.008);
		}
			...
			if (this.metabolics.metabolics_get_percentage('energy') > 50) to_lose = Math.round(this.metabolics.metabolics_get_max_energy() * 0.008);
		}

		if (to_lose){
			//log.info(this+ ' losing energy: '+to_lose);
			this.metabolics.metabolics_lose_energy(to_lose, 1);
		}
	}
...
public function onPlayTimeCheck() {
	...
	if (this.achievements.achievements_has('major_player')) return;

	if (play_time >= 39600) {
		this.achievements.achievements_grant('major_player');
	}
	else {
...
public function isOverDailyFoodLimit(){
	return this.food_today >= (this.metabolics.energy.top * config.food_energy_limit);
}

public function getDailyFoodPercentage(){
...
public function setDoNotDisturb(max_secs){
...
public function clearDoNotDisturb(){
...
public function buildPath(dst, src=null){
	...
	if (!src){
		...
		src_street = Server.instance.apiFindObject(src);
	}
	else{
		src_street = Server.instance.apiFindObject(src);
	...
	if (dst_array){
		...
		for (var i in dst){
			dst_street = Server.instance.apiFindObject(dst[i]);
			if (!dst_street){
		...
		path = Server.instance.apiFindShortestGlobalPath(src, fix_dst);

	}
	else{
		dst_street = Server.instance.apiFindObject(dst);
		...
		path = Server.instance.apiFindGlobalPathX(src, dst);
	...
	var street = Server.instance.apiFindObject(path[path.length-1].tsid);
	if (!street){
...
public function getPathDestTsid(){
	if (!this.map_path){
		return false;
	} else {
...
public function getPathRsp(){
	...
	var dest = last_tsid ? Server.instance.apiFindObject(last_tsid) : null;
...
public function sneeze(){
	this.announcements.announce_sound('SNEEZING_AHCHOO');
	...
	annc.locking = true;
	this.announcements.apiSendAnnouncement(annc);

	this.sendActivity('Achoo!! Argh! Hate that.');
}

public function onCreateItemEvent(details){
	this.location.createItem(details.class_tsid, details.count, details.x, details.y, 100);
}

public function transit_familiar_pay_fare(choice, details){
	if (choice == 'start'){
	...
	if (choice == 'pay'){
		...
		if (loc.transit_pay_fare(this, transit_id)){
			this.announcements.announce_sound('PURCHASE_ITEM');
			return {
...
public function transit_familiar_doors_closed(choice, details){
...
public function transit_disembark_callback(value, details){
...
public function fireflyWhistleSwallowed(details){
	if (details.step == 1){
		this.playHitAnimation('hit2');
		this.events.events_add({callback: 'fireflyWhistleSwallowed', step: 2}, 0.25);
	}
	else if (details.step == 2){
		this.playHitAnimation('hit1');
		this.events.events_add({callback: 'fireflyWhistleSwallowed', step: 3}, 0.25);
	}
	else if (details.step == 3){
		this.playHitAnimation('hit2');
		this.events.events_add({callback: 'fireflyWhistleSwallowed', step: 4}, 0.25);
	}
	else if (details.step == 4){
		this.stopHitAnimation();
		this.playEmotionAnimation('surprise');
		this.announcements.announce_pc_overlay({
			duration: 4000,
			delay_ms: 0,
			locking: true,
			width: 100,
		...
		this.events.events_add({callback: 'fireflyWhistleSwallowed', step: 5}, 2);
	}
	else if (details.step == 5){
		this.achievements.achievements_grant('firefly_whistling');
		this.sendActivity('Oops, you swallowed the whistle! I guess that still counts?');
		this.events.events_add({callback: 'fireflyWhistleSwallowed', step: 6}, 2);
	}
	else if (details.step == 6){
		this.quests.quests_set_flag('firefly_whistle_obtained');
	}
}
...
public function isGreeter(){
	return (this.groups.groups_has(config.greeter_group) && this.items.items_has('bag_greeter_badge', 1));
}

public function isFreeGreeter(){
	if (this.isGreeter()){
		if (this.greeting_muted === undefined) this.greeting_muted = true;
		if (this.is_dead) return false;
		if (this.greeting_muted) return false;
		if (this.games.games_is_playing()) return false;
		if (this.isGuideOnDuty()) return false;
		if (!this.groups.groups_is_in_chat(config.greeter_group)) return false;
...
public function isGreeterTrainee(){
	return this.greeting_graduated ? false : true;
}

public function setGreeterGraduate(){
	this.successful_greets = null;
	this.unsuccessful_greets = null;
	this.greeting_graduated = true;

	this.prompts.prompts_add_simple("You did it! You can now unmute your Greeter Badge and fly solo. Thank you for all you do!");
	this.groups.groups_chat(config.greeter_group, "I just graduated from the greeter training program!");
	if (!config.is_dev) Utils.irc_inject('#greeters', this.label+' just graduated from the greeter training program.');
}

public function greeter_training_callback(value, details){
...
public function ui_callout_cancel(){
	this.apiSendMsg({
		type: "ui_callout_cancel"
	});
}

public function placeInTimeout(args){
...
public function removeFromTimeout(args){
	this.is_in_timeout = null;
...
public function placeInConeOfSilence(args){
...
public function removeFromConeOfSilence(args){
	if (args && args.type == 'help'){
		this.is_in_help_coneofsilence = null;
	}
	else{
		this.is_in_coneofsilence = null;
	}
}
...
public function naughtySplanking(count,spacing){
...
public function doNaughtySplanking(){
	...
	if (this.naughty_splanking.delivered > 0){
		this.metabolics.metabolics_lose_mood(10);
	}
	if (this.naughty_splanking.delivered == this.naughty_splanking.count){
		this.naughty_splanking = null;
		return true;
	}
	...
	this.announcements.apiSendAnnouncement(rsp);
...
public function knowsAboutEnergy(){
...
public function onLabelChanged(){
	this.buddies.buddies_update_reverse_cache();
	this.quickstart_needs_player = null;
...
public function show_rainbow(overlay_key, delay=0){
	...
	if (overlay_key == 'rainbow_secretspot') {
		this.announcements.apiSendAnnouncement(args);
	}
	else {
	...
	switch (overlay_key){
		case 'rainbow_100coinstoday':
			this.announcements.announce_sound('RAINBOW_100COINSTODAY', 0, 0, true);
			break;
		case 'rainbow_goodjob':
			this.announcements.announce_sound('RAINBOW_GOODJOB', 0, 0, true);
			break;
		case 'rainbow_maxrelax':
			this.announcements.announce_sound('RAINBOW_MAXRELAX', 0, 0, true);
			break;
		case 'rainbow_randomkindness':
			this.announcements.announce_sound('RAINBOW_RANDOMKINDNESS', 0, 0, true);
			break;
		case 'rainbow_spindoctor':
			this.announcements.announce_sound('RAINBOW_SPINDOCTOR', 0, 0, true);
			break;
		case 'rainbow_superharvest':
			this.announcements.announce_sound('RAINBOW_SUPERHARVEST', 0, 0, true);
			break;
		case 'rainbow_winner':
			this.announcements.announce_sound('RAINBOW_WINNER', 0, 0, true);
			break;
		case 'rainbow_youdidit':
			this.announcements.announce_sound('RAINBOW_YOUDIDIT', 0, 0, true);
			break;
		case 'rainbow_secretspot':
			this.announcements.announce_sound('RAINBOW_SECRETSPOT', 0, 0, true);
			break;
	}
}
...
*/
public function updateGlitchTrain(location){
	...
	if (chain_length >= required_length) {
		this.achievements.achievements_increment('glitch_train', location);
		this.buffs.buffs_apply('glitch_train');
	}
	else {
		if (this.buffs.buffs_has('glitch_train')){
			this.achievements.achievements_reset_group('glitch_train');	// continuous achievement, reset if conditions not met
			this.buffs.buffs_remove('glitch_train');
		}
	}
}

public function admin_updateHelpCase(args){
...
public function onPlayerCollision(pc){
	log.info(this+' collided with '+pc+'!!!');

	if (this.buffs.buffs_has('feeling_called_love') && !pc.buffs_has('feeling_called_love')){
		this.buffs.buffs_transfer_love(pc);
	}

	this.games.games_handle_player_collision(pc);
}

public function setPlayerCollisions(flag){
	this.p2p_collisions = flag ? 1 : 0;
}

public function hasPlayerCollisions(){
	return this.p2p_collisions ? true : false;
}

//############################################################################

public function hidePack(){
	this.sendMsgOnline({
		'type': 'ui_visible',
		'pack': false
	});
...
public function sorryForDeletingYourTPTarget(){
	...
	var bag = this.mail.mail_get_bag();
	if (!bag) return false;
	...
	this.teleportation.teleportation_give_tokens(7, "Sorry for deleting your Teleportation target.");
	this.mail.mail_add_player_delivery(s.tsid, null, 0, "Here's a special delivery just for you!", config.mail_delivery_time_with_attachment, false);
...
public function getLocationTypeInfo(){
...
public function deletePlayer(){
	...
	this.skills.skills_cancel_learning();

	var house = this.houses.houses_get();
	if (house) {
...
// Utility for testing.
public function clearHellWarning() {
...
public function replyToMsg(msg, ret){
...
public function buildMachine(machine_id, current_part, step){
	if (!this.items.items_has('blockmaker_chassis', 1) ||
	    !this.items.items_has('blockmaker_engine', 1) ||
	    !this.items.items_has('blockmaker_plates', 1)){
		return;
	}
	...
	var stack = this.bag.removeItemStackClass('blockmaker_chassis', 1);
	if (stack){
...
public function overlayDismissed(pc){
	if (this.location.is_newxp){
		this.location.overlayDismissed(pc);
	}
}

public function changeUIComponent(ui_component, enable){
	var args = {type: 'ui_visible'};
	args[ui_component] = enable;
	this.apiSendMsgAsIs(args);
}

public function canJoinTradeChat(){
	return this.imagination.imagination_has_upgrade('trade_channel');
}
...
public function getCraftybot(){
	if (!this.crafty_bot || !this.crafty_bot.tsid) return null;
	return Server.instance.apiFindObject(this.crafty_bot.tsid);
}
...
public function getHQInfo(){
	...
	var today = current_day_key();
	var activity = this.daily_history.daily_history_get_day(today);
	...
	ret.unread_mail = this.mail.mail_count_messages();
	ret.butler_packages = this.butler.has_butler() ? this.butler.getButler().getNumPackagesTotal() : 0;
	ret.currants_to_collect = this.houses.houses_get_currants_to_collect();
	ret.img_to_collect = this.houses.houses_get_img_rewards();
	ret.active_quests = this.quests.countAcceptedQuests();
	...
	ret.skills = this.skills.skills_get_active_progress();
...
public function createCraftybot(x){
