...
public function newxp_move_along(details){
	this.player.prompts.prompts_add_simple('Time to move along. Try going right?', 30);
}

public function newxp_testing_modal_callback(value, details){
	if (value == 'ok'){
		this.player.houses.houses_remove_all();
		this.player.imagination.imagination_reset();
		this.player.stats.stats_reset_imagination();
		this.player.stats.stats_reset_xp();
		this.player.stats.stats_reset_street_history();
		this.player.skills.skills_reset();
		this.player.metabolics.metabolics_set_tank(100);
		this.player.metabolics.metabolics_set_max('energy', 100);
		this.player.metabolics.metabolics_set_energy(95);
		this.player.metabolics.metabolics_set_mood(95);
		this.player.stats.stats_set_quoin_multiplier(1.0);
		this.player.stats.stats_set_currants(0);
		this.player.achievements.achievements_reset();
		this.player.achievements.achievements_reset_daily();
		this.player.quests.quests_reset();
		this.player.counters.counters_reset();
		this.player.physics.setDefaultPhysics();
		this.emptyBag();
		//this.player.furniture.furniture_reset(); // This deletes and re-creates your furn bag, which confuses the client. do the next thing instead
		var furn_bag = this.player.furniture.furniture_get_bag();
		if (furn_bag) furn_bag.emptyBag();
		this.player.physics.physicsReset();
		this.clearPath();

		this.player.announcements.announce_music_stop(this.location.geometry.music_file, 1);
		this.goToNewNewStartingLevel();
	}
}

public function newxpQuestReminder(){
	if (this.player.quests.getQuestStatus('buy_two_bags') != 'done' && this.location.class_tsid == 'newbie_island'){
...
public function newxpComplete(){
	...
	var interior = this.player.houses.houses_get_interior_street();
	if (interior){
...
public function newxpReturnToGentleIsland(){
	...
	this.player.announcements.announce_vog_fade("Though you are on your own now, you are not really alone.//You should find that the other players are pretty helpful, so if you have any questions, just ask.", {width: 600, done_payload: {location_script_name: 'openContacts'}});
}
...
public function newxpGiveGreeterTwig(){
	this.player.announcements.overlay_dismiss('greeter_twig');

	this.player.announcements.announce_vog_fade("It’s a Greeter Twig! You can use it to summon another player to come and say hi.", {css_class: 'nuxp_big', fade_alpha: 0.7, done_payload: {function_name: 'newxpExplainGreeters'}});
	this.player.items.createItemFromFamiliar('greeter_twig', 1);
}

public function newxpExplainGreeters(){
	this.player.announcements.announce_vog_fade("(Greeters are real people — other players who volunteer to welcome new ‘uns — and tend to be nice. Enjoy, and use the twig whenever you want.)", {css_class: 'nuxp_medium', fade_alpha: 0.7});
}
...
public function checkNeighborSignpost(){
	if (this.player.quests.getQuestStatus('leave_gentle_island') == 'done' && this.player.buddies.buddies_count()){
		var exterior = this.player.houses.houses_get_external_street();
		if (exterior){
...
public function onLogin(){
	...
	this.player.familiar.familiar_clean_queue();
	this.player.quests.quests_restart_queue();

	// Clear Zilloween data if necessary
	if (!isZilloween()) {
		this.player.items.clearZilloweenData();
	}
	...
	this.player.imagination.imagination_convert();
	//this.player.imagination.imagination_convert_currants();
}
...
public function onLoggedin(){
	...
	this.player.buffs.buffs_login();
	this.player.houses.houses_login();

	//
	// learn new recipes created since we learned the skill
	//

	this.player.skills.skills_learn_missing_recipes();
	this.player.achievements.achievements_login();
	this.player.quests.quests_login();
	this.player.party.party_login();
	this.player.instances.instances_login();
	this.player.trading.trading_rollback();
	this.player.rewards.rewards_return();
	this.player.conversations.conversations_login();
	...
	if (this.last_overlay_script){
		...
		this.player.announcements.run_overlay_script(this.last_overlay_script);
	...
	var coin_modifier = 0;
	if (this.player.buffs.buffs_has('crazy_coin_collector')) coin_modifier = 50;

	this.player.stats.stats_set_quoins_max_today(this.player.imagination.imagination_get_quoin_limit() + coin_modifier);
	...
	if (this.player.feats.feats_has_unclaimed_rewards()){
		this.player.familiar.familiar_send_alert({
			'callback'	: 'feats_familiar_turnin_do',
			'feat_id'	: this.player.feats.feats_has_unclaimed_rewards()
		});
	}
}
...
public function onLogout(){
	...
	this.player.buffs.buffs_logout();
	this.player.houses.houses_logout();
	this.player.party.party_logout();
	this.player.instances.instances_logout();
	this.player.groups.groups_logout();
	this.player.organizations.organizations_logout();
	this.player.trading.trading_cancel_auto();
	this.player.quests.quests_pause_queue();
	this.player.quests.quests_on_logout();
	...
	if (this.date_last_loggedin && (this.date_last_loggedin > this.date_last_logout || !this.date_last_logout)){
		...
		this.player.achievements.achievements_increment('time_played', day, time_played);
		this.player.counters.counters_increment('time_played', day, time_played);
	...
	this.player.mail.mail_logout();

	this.lastActiveCallback('logout');

	// cancel all action request replies
	this.player.requests.cancelActionRequestReplies();
	...
	if (this.location.isInstance()){
		if (this.location.instance_id.substr(0, 19) == 'underground_puzzle_' || this.location.instance_id.substr(0, 19) == 'ticket_to_paradise_'){
			var exit = this.player.instances.instances_get_exit(this.location.instance_id);
			this.teleportToLocationDelayed(exit.tsid, exit.x, exit.y);
			this.player.prompts.prompts_add_simple("It looks like you ran into some trouble. You've been moved to a safe place.");
		}
	}
}
...
public function didStartStreetMove(){
	...
	if (this.player.buffs.buffs_has('hairball_dash')) {
		this.player.buffs.buffs_hairball_leave_street();
	}
	return true;
}
...
public function teleportToLocation(tsid, x, y, args){
	...
	if (tsid != this.location.tsid && target.instances_instance_me() && !args.ignore_instance_me){
		...
		if (!instance){
			...
			instance = this.player.instances.instances_create(instance_id, target.tsid, instance_options, location_options);

			target.instances_add_instance(instance_id, instance);
		}
		else if (!this.player.instances.instances_has(instance_id)){
			this.player.instances.instances_add(instance_id, instance);
		}
	...
	if (target.getProp('hubid') != this.location.getProp('hubid')){
		this.player.announcements.announce_sound('TELEPORT_AWAY_HUB');
	}else{
		this.player.announcements.announce_sound('TELEPORT_AWAY');
	}
...
public function onTimePlaying(){
	...
	if(!this['!in_house_deco_mode'] && !this.location.isParadiseLocation() && ((!this['!meditating'] || this['!meditating'] != 'transcendental'))) {
		if (!this.location.is_newxp){
			...
			if (this.player.buffs.buffs_has('rookswort')){
				mood_loss_percentage_multiplier = 0.5;
			}

			var max_mood = Math.min(this.player.metabolics.metabolics_get_max_mood(), 2000);

			var mood_loss = Math.max(max_mood * (0.0025 * mood_loss_percentage_multiplier), 1);
			var mood_percentage = this.player.metabolics.metabolics_get_percentage('mood');

			if (mood_percentage > 80){
				mood_loss = Math.max(max_mood * (0.015 * mood_loss_percentage_multiplier), 4);

				if (this.player.imagination.imagination_has_upgrade('periodic_mood_loss_reduced_3')){
					mood_loss = Math.max(max_mood * (0.0075 * mood_loss_percentage_multiplier), 4);
				}else if (this.player.imagination.imagination_has_upgrade('periodic_mood_loss_reduced_2')){
					mood_loss = Math.max(max_mood * (0.010 * mood_loss_percentage_multiplier), 4);
				}else if (this.player.imagination.imagination_has_upgrade('periodic_mood_loss_reduced_1')){
			...
			if (!this.has_done_intro){
				if (this.player.metabolics.metabolics_get_percentage('mood') <= 50) mood_loss = 0;
			}

			if (mood_loss) this.player.metabolics.metabolics_lose_mood(mood_loss, 1);
		}
	}
	...
	if (this.player.metabolics.metabolics_get_percentage('energy') <= 10 && !this.player.buffs.buffs_has('walking_dead') && !this.is_dead){
...
public function getTimePlayed(){
	return this.player.counters.counters_get_group_sum('time_played');
}
...
public function doNewDay(){
	...
	var mood = 0;
	if (this.has_done_intro){
		energy = this.player.metabolics.metabolics_set_energy(this.metabolics.energy.top);
	...
	if (this.isOnline() && (previous_time_played + session_time_played) >= 3600 && this.has_done_intro){
		...
		var rsp = {
			...
			"stats_yesterday"	: {
				energy: intval(this.player.daily_history.daily_history_get(yesterday_key, 'energy_consumed')) * -1,
				imagination: intval(this.player.daily_history.daily_history_get(yesterday_key, 'imagination'))
			},
			"stats_today"		: {
				energy: energy,
				mood: mood
		...
		var levels = intval(this.player.daily_history.daily_history_get(yesterday_key, 'level_up'));
		if (levels) rsp.new_level = this.player.stats.stats_get_level();

		var quests = this.player.daily_history.daily_history_get(yesterday_key, 'quests_completed');
		if (num_keys(quests)){
			rsp.complete_quests = [];
			for (var i in quests){
				var q = this.player.quests.getQuestInstance(quests[i]);
				if (q){
		...
		var collections = this.player.daily_history.daily_history_get(yesterday_key, 'collections');
		if (num_keys(collections)){
			rsp.complete_collections = [];
			for (var i in collections){
				var c = this.player.achievements.achievements_get(collections[i]);
				if (c){
					rsp.complete_collections.push({name: c.name, tsid: this.player.achievements.achievement_get_trophy(collections[i]).class_tsid});
				}
			}
		}

		var skills = this.player.daily_history.daily_history_get(yesterday_key, 'skills_learned');
		if (num_keys(skills)){
			rsp.new_skills = [];
			for (var i in skills){
				rsp.new_skills.push({name: this.player.skills.skills_get_name(skills[i]), tsid: skills[i]});
			}
		}

		var achievements = this.player.daily_history.daily_history_get(yesterday_key, 'achievements');
		if (num_keys(achievements)){
			rsp.new_badges = [];
			for (var i in achievements){
				var a = this.player.achievements.achievements_get(achievements[i]);
				if (a){
					rsp.new_badges.push({
						name: a.name,
						tsid: achievements[i],
		...
		this.player.daily_history.daily_history_archive(yesterday_key);
		this.player.daily_history.daily_history_reset();
	}
	...
	if (this.stats.quoins_today.value >= 100){
		...
		this.player.achievements.achievements_set('coin_count', 'days_maxed', 0);
	}

	this.player.stats.stats_set_quoins_today(0);

	// fix broken quoin upgrades:
	this.player.stats.stats_set_quoins_max_today(this.player.imagination.imagination_get_quoin_limit());

	if (this.player.buffs.buffs_has('crazy_coin_collector')){
		this.player.buffs.buffs_remove('crazy_coin_collector');
	}

	// Reset daily rube trades
	this.player.stats.stats_set_rube_trades(0);
	this.player.stats.stats_set_rube_lure_disabled(false);

	// Food counter
	this.food_today = 0;

	// Meditation counters
	this.player.stats.stats_set_meditation_today(0);
	...
	if (this.butlers_zombied_today) {
		...
		this.player.quests.quests_set_counter("butlers_zombied", 0);
	}

	// Reset daily favor limits
	for(var i = 0; i < config.giants.length; ++i) {
		var giant = config.giants[i];
		this.init_prop('daily_favor', giant, 0, 0, this.player.stats.stats_get_max_favor(giant));
	...
	this.player.teleportation.teleportation_reset_counters();

	// Mining gem drop overrides
	this.player.skill_packages.removeSkillPackageOverride('mining');
	this.player.skill_packages.removeSkillPackageOverride('mining_fancypick');

	if (this.player.buffs.buffs_has('impervious_miner')){
		this.player.skill_packages.addSkillPackageOverride('mining', {energy_cost: 0});
...
public function deleteMe(){
		...
		for (var i in this.friends){
			if (this.friends[i].pcs){
				for (var j in this.friends[i].pcs){
					...
					this.player.buddies.removeBuddy(this.friends[i].pcs[j].tsid);
				}
			}
		}
	...
	this.player.trading.trading_reset();
	...
	this.player.rewards.rewards_reset();
	this.player.prompts.prompts_delete();
	this.player.auctions.auctions_delete();
	this.player.houses.houses_remove_all();
	this.player.familiar.familiar_delete();
	this.player.quests.quests_delete();
	this.player.skills.skills_delete();
	this.player.stats.stats_delete();
	this.player.making.making_delete();
	this.player.buffs.buffs_delete();
	this.player.achievements.achievements_delete_all();
	this.player.instances.instances_delete_all();
	this.player.groups.groups_delete_all();
	this.player.organizations.organizations_delete_all();
	this.player.mail.mail_delete();
	this.player.conversations.conversations_delete();
...
public function onCreate(){
	...
	if (this.skip_newux){
		this.player.stats.stats_set_currants(1000);
		this.player.metabolics.metabolics_set_mood(80);
		this.player.metabolics.metabolics_set_energy(80);
		this.player.stats.stats_add_xp(100);

		this.player.items.createItem('hoe', 1);
		this.player.items.createItem('watering_can', 1);
		this.player.items.createItem('sammich', 3);
		this.player.items.createItem('papl_upside_down_pizza', 3);
		this.player.items.createItem('common_crudites', 3);
		this.player.items.createItem('milk_butterfly', 3);
		this.player.items.createItem('bubble_tea', 3);
		this.player.items.createItem('cloud_11_smoothie', 3);
	...
	this.player.stats.stats_set_currants(0); // formerly 100: http://bugs.tinyspeck.com/10801
	this.player.metabolics.metabolics_set_mood(55);
	this.player.metabolics.metabolics_set_energy(55);
...
public function croak(){
	...
	this.player.metabolics.metabolics_lose_mood(this.player.metabolics.metabolics_get_mood());
	this.player.metabolics.metabolics_lose_energy(this.player.metabolics.metabolics_get_energy());
	...
	this.player.achievements.achievements_increment('player', 'deaths');


	if (this.map_path) this.previous_map_path = this.map_path;

	//this.player.quests.quests_offer('you_croaked', true);

	if (this.player.buffs.buffs_has('no_no_powder_crash')){
		this.player.buffs.buffs_remove('no_no_powder_crash');
	}

	if (this.player.buffs.buffs_has('no_no_powder')){
		this.player.buffs.buffs_remove('no_no_powder');
	}
...
public function resurrect(){
	...
	this.player.announcements.announce_music_stop('HELL', 2);

	if (this.player.buffs.buffs_has('pooped')){
		this.player.metabolics.metabolics_set_energy(this.metabolics.energy.top * 0.05);
	}else{
		this.player.metabolics.metabolics_set_energy(this.metabolics.energy.top * 0.10);
	}
	if (this.player.metabolics.metabolics_get_mood() > (this.metabolics.mood.top / 2)) this.player.metabolics.metabolics_set_mood(this.metabolics.mood.top / 2);

	var dest;
	var prev;
	if (this.location.isInstance('hell_one')){
		prev = this.player.instances.instances_get_exit('hell_one');
	...
	this.player.quests.quests_set_flag('resurrected');
...
public function recognizeDeath(type, recognizer) {
	...
	if (this.isOnline()) {
		if (this.location.isInHellOne() && this.player.quests.getQuestStatus('hell_hole') == 'none' && this.player.quests.getQuestStatus('hell_quest') == 'none'){
			this.player.quests.quests_offer('hell_hole', true);
		}else if (this.location.isInPurgatoryHub() && this.player.quests.getQuestStatus('hell_quest') == 'none'){
			this.player.quests.quests_offer('hell_quest', true);
		}else{
			var activityText = "";
			...
			var remaining = this.player.items.createItemFromFamiliar('drink_ticket', 1);
			if (remaining) {
...
public function goToNewNewStartingLevel(){
	...
	for (var i in this.instances.instances){
		this.player.instances.instances_left(i);
	...
	var instance = this.player.instances.instances_create(instance_id, tsid, {no_auto_return: true, location_type: instance_id});

	// Create the second instance and add it to the player
	var instance2 = this.player.instances.instances_create('newxp_training1', config.newxp_locations['newxp_training1'], {no_auto_return: true, location_type: 'newxp_training1'});

	// Create the third instance and add it to the player
	var instance3 = this.player.instances.instances_create('newxp_training2', config.newxp_locations['newxp_training2'], {no_auto_return: true, location_type: 'newxp_training2'});

	// Go to the first instance
	this.player.instances.instances_enter(instance_id, 609, -6100, true);
}

public function goToFallingLevel(exit){

	var tsid = config.is_dev ? 'LHH12F9DO801RUL' : 'LLI12DVTLA01HT5';
	var instance = this.player.instances.instances_create('falling', tsid, {instance_label: 'You Feel Woooooozy...', no_auto_return: true});
	if (instance){
		if (exit && exit.tsid){
		...
		this.player.instances.instances_enter('falling', 0, -10000);
	}
}
...
public function resetForTesting(skip){
	...
	this.player.houses.houses_remove_all();
	this.player.familiar.familiar_reset();
	this.player.quests.quests_reset();
	this.player.skills.skills_reset();
	this.player.stats.stats_delete();
	this.player.making.making_reset();
	this.player.buffs.buffs_reset();
	this.player.achievements.achievements_reset();
	this.player.trophies.trophies_reset();
	this.player.instances.instances_reset();
	this.player.teleportation.teleportation_reset();
	this.player.daily_history.daily_history_reset();
	//this.player.jobs.jobs_reset(); // Maybe not do this? Since we're not resetting jobs
	this.player.auctions.auctions_reset(true);
	this.player.rewards.rewards_reset();
	this.player.mail.mail_reset();
	this.player.conversations.conversations_reset();
	this.player.trading.trading_reset();
	this.player.prompts.prompts_reset();
	this.player.buddies.buddies_reset_cache();
	this.player.counters.counters_reset();
	this.player.imagination.imagination_reset();
	...
	if (tokens_to_refund){
		this.player.teleportation.teleportation_give_tokens(tokens_to_refund, "Imbued Teleportation Script refund");
	}

	// Restore items
	for (var i in items_to_restore){
		this.player.items.createItemFromFamiliar(items_to_restore[i], 1);
	}
...
public function performPostProcessing(msg){
	...
	if (this['!meditating']){
		...
		if (interrupt && !in_array(msg.type, meditation_ok_copy) && !in_array(msg.verb, this.meditation_ok_verbs)){
			...
			var orbs = this.player.items.get_stacks_by_class('focusing_orb');
			for (var i in orbs){
	...
	if (msg.type == 'move_xy' && this['!moved_lat'] >= 150){
		if (this['!dismiss_overlay']){
			this.player.events.events_add({overlay: this['!dismiss_overlay'], callback: 'dismiss_overlay_event'}, 1);
			delete this['!dismiss_overlay'];
		}
		else if (this.run_overlay_onmove){
			this.player.events.events_add({overlay: this.run_overlay_onmove, callback: 'run_overlay_event'}, 1);
...
public function make_hash_with_location(){
	var out = {
		...
		rs	: this.player.rook.isRooked(),
		is_admin	: (this.is_god || this.is_help) ? true : false,
		is_guide	: this.is_guide ? true : false,
		home_info	: this.player.houses.houses_get_login_new()
	};

	out['physics_adjustments'] = this.player.physics.reducePhysicsAdjustmentsToMinumum(this.player.physics.getStackedPhysics());

	var props = this.player.avatar.avatar_get_pc_msg_props();
	for (var i in props) out[i] = props[i];

	this.player.games.games_add_msg_props(out);
...
public function getCameraAbilities(){
	...
	if (!this.player.skills.skills_has('eyeballery_1')){
		range = 400;
	...
	if (!this.player.imagination.imagination_has_upgrade('snapshotting')){
		can_snap = false;
...
public function inputBoxResponse(msg){
	...
	if (msg.itemstack_tsid){
		...
		if (msg.value){
			...
			var org = this.player.organizations.organizations_get();
			if (org){
...
public function onPeriodicEnergyLoss(){
	...
	if (!this['!in_house_deco_mode'] && !this.location.isParadiseLocation() &&  ((!this['!meditating'] || this['!meditating'] != 'transcendental'))){
		...
		if (this.has_done_intro){
			...
			to_lose = Math.round(this.player.metabolics.metabolics_get_max_energy() * 0.008);
		}
			...
			if (this.player.metabolics.metabolics_get_percentage('energy') > 50) to_lose = Math.round(this.player.metabolics.metabolics_get_max_energy() * 0.008);
		}

		if (to_lose){
			//log.info(this+ ' losing energy: '+to_lose);
			this.player.metabolics.metabolics_lose_energy(to_lose, 1);
		}
	}
...
public function onPlayTimeCheck() {
	...
	if (this.player.achievements.achievements_has('major_player')) return;

	if (play_time >= 39600) {
		this.player.achievements.achievements_grant('major_player');
	}
	else {
...
public function sneeze(){
	this.player.announcements.announce_sound('SNEEZING_AHCHOO');
...
public function fireflyWhistleSwallowed(details){
	if (details.step == 1){
		...
		this.player.events.events_add({callback: 'fireflyWhistleSwallowed', step: 2}, 0.25);
	}
	else if (details.step == 2){
		this.playHitAnimation('hit1');
		this.player.events.events_add({callback: 'fireflyWhistleSwallowed', step: 3}, 0.25);
	}
	else if (details.step == 3){
		this.playHitAnimation('hit2');
		this.player.events.events_add({callback: 'fireflyWhistleSwallowed', step: 4}, 0.25);
	}
	else if (details.step == 4){
		this.stopHitAnimation();
		this.playEmotionAnimation('surprise');
		this.player.announcements.announce_pc_overlay({
			duration: 4000,
			delay_ms: 0,
			locking: true,
			width: 100,
		...
		this.player.events.events_add({callback: 'fireflyWhistleSwallowed', step: 5}, 2);
	}
	else if (details.step == 5){
		this.player.achievements.achievements_grant('firefly_whistling');
		this.sendActivity('Oops, you swallowed the whistle! I guess that still counts?');
		this.player.events.events_add({callback: 'fireflyWhistleSwallowed', step: 6}, 2);
	}
	else if (details.step == 6){
		this.player.quests.quests_set_flag('firefly_whistle_obtained');
	}
}
...
public function isGreeter(){
	return (this.player.groups.groups_has(config.greeter_group) && this.player.items.items_has('bag_greeter_badge', 1));
}
...
public function isFreeGreeter(){
	if (this.isGreeter()){
		...
		if (this.player.games.games_is_playing()) return false;
...
public function setGreeterGraduate(){
	...
	this.player.prompts.prompts_add_simple("You did it! You can now unmute your Greeter Badge and fly solo. Thank you for all you do!");
	this.player.groups.groups_chat(config.greeter_group, "I just graduated from the greeter training program!");
...
public function show_rainbow(overlay_key, delay=0){
	...
	switch (overlay_key){
		case 'rainbow_100coinstoday':
			this.player.announcements.announce_sound('RAINBOW_100COINSTODAY', 0, 0, true);
			break;
		case 'rainbow_goodjob':
			this.player.announcements.announce_sound('RAINBOW_GOODJOB', 0, 0, true);
			break;
		case 'rainbow_maxrelax':
			this.player.announcements.announce_sound('RAINBOW_MAXRELAX', 0, 0, true);
			break;
		case 'rainbow_randomkindness':
			this.player.announcements.announce_sound('RAINBOW_RANDOMKINDNESS', 0, 0, true);
			break;
		case 'rainbow_spindoctor':
			this.player.announcements.announce_sound('RAINBOW_SPINDOCTOR', 0, 0, true);
			break;
		case 'rainbow_superharvest':
			this.player.announcements.announce_sound('RAINBOW_SUPERHARVEST', 0, 0, true);
			break;
		case 'rainbow_winner':
			this.player.announcements.announce_sound('RAINBOW_WINNER', 0, 0, true);
			break;
		case 'rainbow_youdidit':
			this.player.announcements.announce_sound('RAINBOW_YOUDIDIT', 0, 0, true);
			break;
		case 'rainbow_secretspot':
			this.player.announcements.announce_sound('RAINBOW_SECRETSPOT', 0, 0, true);
			break;
	}
}
...
public function updateGlitchTrain(location){
	...
	if (chain_length >= required_length) {
		this.player.achievements.achievements_increment('glitch_train', location);
		this.player.buffs.buffs_apply('glitch_train');
	}
	else {
		if (this.player.buffs.buffs_has('glitch_train')){
			this.player.achievements.achievements_reset_group('glitch_train');	// continuous achievement, reset if conditions not met
			this.player.buffs.buffs_remove('glitch_train');
		}
	}
}
...
public function onPlayerCollision(pc){
	...
	if (this.player.buffs.buffs_has('feeling_called_love') && !pc.buffs_has('feeling_called_love')){
		this.player.buffs.buffs_transfer_love(pc);
	}

	this.player.games.games_handle_player_collision(pc);
}
...
public function sorryForDeletingYourTPTarget(){
	...
	var bag = this.player.mail.mail_get_bag();
	if (!bag) return false;
	...
	this.player.teleportation.teleportation_give_tokens(7, "Sorry for deleting your Teleportation target.");
	this.player.mail.mail_add_player_delivery(s.tsid, null, 0, "Here's a special delivery just for you!", config.mail_delivery_time_with_attachment, false);
...
public function deletePlayer(){
	...
	this.player.skills.skills_cancel_learning();

	var house = this.player.houses.houses_get();
	if (house) {
...
public function buildMachine(machine_id, current_part, step){
	if (!this.player.items.items_has('blockmaker_chassis', 1) ||
	    !this.player.items.items_has('blockmaker_engine', 1) ||
	    !this.player.items.items_has('blockmaker_plates', 1)){
		return;
	}
...
public function canJoinTradeChat(){
	return this.player.imagination.imagination_has_upgrade('trade_channel');
}
...
public function getHQInfo(){
	...
	var today = current_day_key();
	var activity = this.player.daily_history.daily_history_get_day(today);
	...
	ret.unread_mail = this.player.mail.mail_count_messages();
	ret.butler_packages = this.player.butler.has_butler() ? this.player.butler.getButler().getNumPackagesTotal() : 0;
	ret.currants_to_collect = this.player.houses.houses_get_currants_to_collect();
	ret.img_to_collect = this.player.houses.houses_get_img_rewards();
	ret.active_quests = this.player.quests.countAcceptedQuests();
	...
	ret.skills = this.player.skills.skills_get_active_progress();
