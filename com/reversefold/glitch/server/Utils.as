...
public static function JSON_stringify(value, space=null) {
	...
	return _JSON_str('x', {'x': value}, indent, '');
}


// internal guts below here

public static function _JSON_quote(string) {
...
public static function _JSON_str(key, holder, indent, gap) {
		...
		case 'object':
			...
			if (value instanceof Array) {
				...
				for (i = 0; i < length; i += 1) {
					partial[i] = _JSON_str(i, value, indent, gap) || 'null';
				}
			...
			for (k in value) {
				...
				v = _JSON_str(k, value, indent, gap);
				if (v) {
					partial.push(_JSON_quote(k) + (gap ? ': ' : ':') + v);
				}
				//}
			}
