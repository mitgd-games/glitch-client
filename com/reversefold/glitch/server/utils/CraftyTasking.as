package com.reversefold.glitch.server.utils {
	...
	public class CraftyTasking {
		private static var log : Logger = Log.getLogger("server.utils.CraftyTasking");

public static var CRAFTYTASKING_OUPUT_TIMING_DATA = true;

public static function craftytasking_test(){
	var all_specs = Server.instance.apiFindItemPrototype('catalog_craftytasking').craftytasking_specs;
	log.info('CRAFTY -- all_specs: '+all_specs['sammich']);
}

public static function craftytasking_categories(){
...
public static function craftytasking_category_items(crafty_bot, category){
	
	var cache_data = null;
	if (crafty_bot){
		cache_data = craftytasking_build_cache(crafty_bot.getIngredientSource()); // RVRS: originally crafty_data, typo?		
	}
	...
	// Get tags
	//
	var tags = Server.instance.apiFindItemPrototype('catalog_tags');
...
/*
public static function craftytasking_category_items(category){
var tags = Server.instance.apiFindItemPrototype('catalog_tags');

var full_tags = {};
for (var i in tags.item_tags[category]){
full_tags[tags.item_tags[category][i]] = Server.instance.apiFindItemPrototype(tags.item_tags[category][i]).name_single;
};
...
public static function craftytasking_get_static_build_spec(class_tsid)
{
	return Server.instance.apiFindItemPrototype('catalog_craftytasking')['craftytasking_specs'][class_tsid];
}


public static function craftytasking_get_build_spec(crafty_bot, class_tsid, req_count, use_current_ingredients){
	//
...
public static function craftytasking_get_build_spec_recurs(crafty_bot, class_tsid, req_count, use_current_ingredients, claimed_ingredients, required_elements, crafting_items, deep, cache_data){
			...
			if (recipe && recipe[class_tsid]){
				...
				if (recipe[class_tsid]['tool']) {
					if (recipe[class_tsid]['tool'] == 'self'){
						...
						if (tools[0]){
							var tool = Server.instance.apiFindItemPrototype(tools[0]);
...
//
public static function craftytasking_get_build_spec_list(spec, depth){
	var list = {};
	depth++;
...
//
public static function craftytasking_get_build_element_spec(crafty_bot, required_elements, claimed_ingredients){
	var ret = {};
...
public static function craftytasking_get_required_rocks_for_elements(required_elements, active_element, rock_left, shiny_multiplier, blue_multiplier, red_multiplier, green_multiplier){
	var rock_used = 0;
...
public static function craftytasking_limited_spec(spec, max_steps){
...
public static function craftytasking_limited_spec_recurs(spec, max_steps){
	var limited_spec = [];
...
public static function craftytasking_recipe_request(crafty_bot, msg){
	var rsp = {};
...
public static function craftytasking_build_sequence(crafty_bot, spec, count, holder=null){
...
public static function craftytasking_get_sequence_status(sequence){
...
public static function craftytasking_build_sequence_step(crafty_bot, spec, count, ignore_source_item, craft_sequence, ingredient_source_shopping_list, external_crafted_items, holder){
	var container = null;
	if (crafty_bot.isInLocation()){
		container = crafty_bot.getLocation();
	}else{
...
public static function craftytasking_build_shopping_list_step(crafty_bot, spec, multiplier, ignore_source_item, shopping_list, holder){
...
public static function craftytasking_build_cache(container){
	var cache_data = {};
...
public static function craftytasking_get_buildable_count(container, class_tsid){
...
public static function craftytasking_get_buildable_count_recurs(class_tsid, cache_data, depth){
...
public static function craftytasking_find_craftable_recipes(crafty_bot, use_current_ingredients, category){
	...
	if (category){
	prot = Server.instance.apiFindItemPrototype(tmp_recipe.outputs[k][0]);
	...
	var recipes = Server.instance.apiFindItemPrototype('catalog_recipes').recipes;
...
apiResetThreadCPUClock();
var fooCpuTime = Server.instance.apiResetThreadCPUClock("crafty_bot");
...
public static function get_string_list_from_class_tsid(class_tsid, count){
	var string = '';
	var items = class_tsid.split('|');
	var first = true;
	for (var i in items){
		var item = Server.instance.apiFindItemPrototype(items[i]);
		if (string == ''){
			if (count == 1){
				string += item.article;	
			}else{
